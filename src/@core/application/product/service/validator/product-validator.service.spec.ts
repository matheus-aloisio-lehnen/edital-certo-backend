...
describe("ProductValidatorService", () => {
    ...
    describe("validatePlanKeys", () => {
        it("should not throw error when all plan keys are unique", () => {
            const keys = [planKey.free, planKey.start, planKey.pro];
            expect(() => service.validatePlanKeys(keys)).not.toThrow();
        });

        it("should throw AppException when there are duplicated plan keys", () => {
    ...
    describe("validatePriceKeys", () => {
        it("should not throw error when all price keys are unique", () => {
            const keys = [priceKey.startMonthlyBrl, priceKey.startYearlyBrl];
            expect(() => service.validatePriceKeys(keys)).not.toThrow();
        });

        it("should throw AppException when there are duplicated price keys", () => {
    ...
    describe("validateDiscountKeys", () => {
        it("should not throw error when all discount keys are unique", () => {
            const keys = ["DISCOUNT_1", "DISCOUNT_2"];
            expect(() => service.validateDiscountKeys(keys)).not.toThrow();
        });

        it("should throw AppException when there are duplicated discount keys", () => {
    ...
    describe("validateFeaturesKeys", () => {
        it("should not throw error when all feature keys are unique", () => {
            const keys = [featureKey.aiHelp, featureKey.calendar];
            expect(() => service.validateFeaturesKeys(keys)).not.toThrow();
        });

        it("should throw AppException when there are duplicated feature keys", () => {
