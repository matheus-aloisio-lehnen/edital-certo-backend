...
import { OpenTelemetryLogger } from "./opentelemetry-logger.adapter";
import { createConfigServiceMock } from '@mock/tests.mock';

describe('Logger', () => {
    let service: OpenTelemetryLogger;
    ...
    beforeEach(() => {
        ...
        service = new OpenTelemetryLogger(MockConfigServe);
    });
    ...
    it('write should not export when observability are disabled', () => {
        MockConfigServe = createConfigServiceMock({
            observability: { logs: false, metric: true, trace: true, endpoint: "" },
        });
        service = new OpenTelemetryLogger(MockConfigServe);
